...
export function VehiclesManager() {
  ...
  async function loadVehicles() {
    ...
    const { data, error: loadError } = await supabase
      .from("mezzi")
      .select(
        "id,targa,marca,modello,reparto,responsabile,detentore_chiavi,stato,motivo_stato,attivo,note"
      )
  ...
  const filteredVehicles = useMemo(() => {
    ...
    return vehicles.filter((vehicle) => {
      ...
      const matchesStatus =
        statusFilter === "tutti" || vehicle.stato === statusFilter;
  ...
  return (
    <>
      ...
      <section className="panel vehiclePanel">
        <div className="toolbar">
          <label className="searchBox">
            ...
            <input
              ...
              placeholder="Cerca targa, marca, modello o reparto"
            />
          </label>
        ...
        {loading ? (
          <div className="emptyState">Caricamento mezzi…</div>
        ) : filteredVehicles.length === 0 ? (
          <div className="emptyState">
            Nessun mezzo trovato. Premi “Aggiungi mezzo” per inserire il primo.
          </div>
        ) : (
              ...
              <tbody>
                {filteredVehicles.map((vehicle) => (
                  <tr key={vehicle.id}>
                    <td>
                      <Link
                        href={`/mezzi/${vehicle.id}`}
                        title={`Apri la scheda del mezzo ${vehicle.targa}`}
                        style={{
                          color: "inherit",
                    ...
                    <td>
                      <span
                        className={`statusBadge status-${vehicle.stato}`}
                      >
        ...
        >
          ...
          >
            <div className="modalHeader">
              <div>
                <p className="eyebrow">Anagrafica mezzo</p>
                <h3>{editingId ? "Modifica mezzo" : "Nuovo mezzo"}</h3>
              </div>
            ...
            <form onSubmit={saveVehicle} className="vehicleForm">
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setForm({
                      ...form,
                      responsabile: event.target.value
                    })
                  }
                />
              </label>
              ...
              <label>
                Detentore chiavi
                <input
                  value={form.detentore_chiavi ?? ""}
                  onChange={(event) =>
                    setForm({
                      ...form,
                      detentore_chiavi: event.target.value
                    })
                  }
                />
              </label>
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setForm({
                      ...form,
                      stato: event.target.value as VehicleStatus
                    })
                  }
                >
                  {Object.entries(statusLabels).map(([value, label]) => (
              ...
              <label>
                Motivo stato
                <input
                  value={form.motivo_stato ?? ""}
                  onChange={(event) =>
                    setForm({
                      ...form,
                      motivo_stato: event.target.value
                    })
                  }
                />
              </label>
